...
func (rm *RegistryManager) InstallDriver(driverName string) error {
	...
	fmt.Printf("Downloading %s driver v%s for %s...\n", driverName, driverInfo.Version, platform)
	if err := rm.downloadFile(platformInfo.URL, driverPath); err != nil {
	...
	fmt.Printf("Successfully installed %s driver v%s\n", driverName, driverInfo.Version)
	return nil
}
